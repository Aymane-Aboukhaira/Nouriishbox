...
function SignUpContent() {
  ...
  // Only redirect if already authenticated on page load (e.g. user visits signup while logged in)
  useEffect(() => {
  ...
  const onSubmit = (data: SignUpForm) => {
    ...
    // signUp() is synchronous — it sets state immediately, then we navigate
    signUp({ name: data.name, email: data.email, phone: data.phone, address: data.address });
    // Use replace so the back button doesn't return to signup
    if (plan) {
