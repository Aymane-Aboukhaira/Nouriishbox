    ...
    <div className="min-h-screen grid lg:grid-cols-2 overflow-hidden bg-background">
      ...
      <div className="hidden lg:flex flex-col justify-between bg-primary p-16 relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          >
            Eat Like Your Body <br /> 
            <span className="italic opacity-90">Was Designed To.</span>
          </motion.h1>
        </div>
        ...
        >
          <div className="relative aspect-square max-w-sm ml-auto rounded-[32px] border-2 border-dashed border-background/20 bg-background/5 flex items-center justify-center overflow-hidden">
            <div className="text-center">
              ...
              <p className="font-sans text-[10px] font-bold uppercase tracking-[0.2em] text-background/30 px-6">Product Snapshot Placeholder</p>
            </div>
          </div>
        </motion.div>
      </div>
      ...
      <div className="flex flex-col justify-start lg:justify-center px-8 sm:px-12 md:px-16 lg:px-24 py-16 min-h-screen overflow-y-auto">
        ...
        >
          <div className="mb-10">
            <h2 className="font-serif text-4xl text-text-primary mb-3">Create Account</h2>
            <p className="text-text-muted font-sans text-base">Your plan data is saved — takes 30 seconds.</p>
          </div>

          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div className="grid grid-cols-1 gap-4">
              <div className="space-y-1.5">
                <label className="block text-xs font-bold text-text-primary px-1">Full Name</label>
              ...
              <div className="space-y-1.5">
                <label className="block text-xs font-bold text-text-primary px-1">Email Address</label>
                <Input {...register("email")} type="email" placeholder="you@example.com" error={!!errors.email} />
                {errors.email && <p className="text-accent text-[10px] font-bold px-4 uppercase tracking-widest mt-1">{errors.email.message}</p>}
              </div>

              <div className="space-y-1.5">
                <label className="block text-xs font-bold text-text-primary px-1">Phone Number</label>
              ...
              <div className="space-y-1.5">
                <label className="block text-xs font-bold text-text-primary px-1">Delivery Address</label>
                <Input {...register("address")} placeholder="Street, neighbourhood, Casablanca" error={!!errors.address} />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="block text-xs font-bold text-text-primary px-1">Password</label>
                ...
                <div className="space-y-1.5">
                  <label className="block text-xs font-bold text-text-primary px-1">Confirm</label>
                  <div className="relative">
            ...
            {formError && (
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-accent/5 border border-accent/20 rounded-[20px] p-4 text-sm text-accent font-semibold flex items-center gap-3 mt-4">
                ...
                <span>{formError} {formError.includes("already exists") && <Link href="/auth/signin" className="underline font-bold">Sign in instead?</Link>}</span>
              </motion.div>
            )}
            ...
            <Button variant="primary" size="lg" className="w-full h-14 text-sm font-bold uppercase tracking-widest mt-6" type="submit" disabled={isSubmitting}>
              {isSubmitting ? <Loader2 size={20} className="animate-spin" /> : <>Create Account <ArrowRight size={16} className="ml-2" /></>}
            </Button>
          </form>

          <div className="my-8 flex items-center gap-4">
            <div className="flex-1 h-[1px] bg-border" />
            <span className="text-[9px] font-bold text-text-muted uppercase tracking-[0.2em] whitespace-nowrap">Or continue with</span>
            <div className="flex-1 h-[1px] bg-border" />
          </div>

          <Button variant="secondary" size="lg" className="w-full h-12 bg-white border-border text-text-primary hover:border-primary font-bold text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 group" onClick={() => toast.info("Google login coming soon")}>
          ...
          <p className="text-center text-xs font-medium text-text-muted mt-8">
            ...
            <Link href="/auth/signin" className="text-accent underline underline-offset-4 hover:opacity-80 transition-opacity font-bold ml-1">Sign In</Link>
          </p>
        </motion.div>
      </div>
    </div>
